...
class LogisticRegression(Problem):
    ...
    def __init__(
        self,
        problem_type: str = "mushrooms"
	) -> None:
        super().__init__(
            info=f"Logistic Regression problem on {problem_type} dataset",
            func=self.f
        )
        ...
        if problem_type == "mushrooms":
            ...
            self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(X, y, train_size=0.8, test_size=0.2, shuffle=True)
